...
def generate_personal_details_latex(details: PersonalDetails, logo_path: str) -> str:
    # This function now perfectly replicates the header from your reference image.
    return f"""
\\begin{{tabular*}}{{\\textwidth}}{{l@{{\\extracolsep{{\\fill}}}}r}}
    \\raisebox{{-0.0\\height}}{{\\includegraphics[height=1.5cm]{{{logo_path}}}}} &
    \\begin{{tabular}}[b]{{l}}
...
def generate_scholastic_latex(achievements: List[ScholasticAchievement]) -> str:
    # Reverting to the simpler, robust \textls structure from your code
    items = "".join([f"    \\item\\textls[10]{{{{{sanitize(ach.text)}}}}}\n" for ach in achievements])
...
{points_latex}\\end{{itemize}}
"""
        # Add final spacing between the whole section and the next one
        if i == len(projects) - 1: latex_string += "\\vspace{-6mm}\n"
    return latex_string
...
async def generate_pdf(resume_data: ResumeData):
    # This is a placeholder for the IITB logo. You need to add the logo file to your project.
    iitb_logo_path = "iitb_logo.png" # Make sure you have this file in your backend directory
    final_iitb_logo_path = os.path.join("/app", iitb_logo_path).replace("\\", "/")

    try:
        # The main logic, adapted for the new generator functions
        ...
        tex_filepath = f"/app/{session_id}.tex"
        with open(tex_filepath, "w", encoding='utf-8') as f: f.write(latex_template)
        
        process = subprocess.run(['pdflatex', '-output-directory=/app', '-interaction=nonstopmode', tex_filepath], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
        pdf_filepath = f"/app/{session_id}.pdf"
        if not os.path.exists(pdf_filepath):
            log_filepath = f"/app/{session_id}.log"
